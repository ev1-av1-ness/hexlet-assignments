...
public class AppTest {
    ...
    void testFilterByFirstAndLastNameAndEmailAndProfessionAndGender() throws Exception {
        MockHttpServletResponse response = mockMvc
                .perform(get("/users/dt?firstName=be&lastName=rr&email=dmer&profession=Coordinator&gender=Male"))
                .andReturn()
                .getResponse();
        ...
        User actualUser = actualUsers.get(0);
        assertThat(actualUsers.size()).isEqualTo(1);
        assertThat(actualUser.getFirstName()).isEqualTo("Darbee");
        assertThat(actualUser.getLastName()).isEqualTo("Merrett");
        assertThat(actualUser.getEmail()).isEqualTo("dmerrett16@oakley.com");
        assertThat(actualUser.getGender()).isEqualTo("Male");
        assertThat(actualUser.getProfession()).isEqualTo("Payment Adjustment Coordinator");
    }
    // END
}
